...
class PhishingFeatures:
    ...
    # 1. Deception & Brand
    corporate_lookalike_distance: float = -1.0
    brand_homoglyph_detected: bool = False
    url_shortener_used: bool = False
    
    # 2. Time-Based Volatility
    passive_dns_first_seen: str = ""
    domain_expiration_duration_days: int = -1
    certificate_transparency_velocity: int = -1
...
def sanitize_indicator(indicator: str) -> str:
    ...
    # Strip any lingering paths or trailing slashes
...
def fetch_virustotal_data(indicator: str, ind_type: str, features: PhishingFeatures):
    """Fetches Reputation and WHOIS from VirusTotal."""
    if not VT_API_KEY: return
    headers = {"x-apikey": VT_API_KEY}
    endpoint = "ip_addresses" if ind_type == "IP" else "domains"
    
    try:
        resp = requests.get(f"https://www.virustotal.com/api/v3/{endpoint}/{indicator}", headers=headers, timeout=10)
        if resp.status_code == 200:
            ...
            features.confidence_score_stix = max(0, min(100, int((rep + 100) / 2)))

            # Domain Specifics (Expiration)
            if ind_type == "DOMAIN":
...
def fetch_otx_data(indicator: str, ind_type: str, features: PhishingFeatures):
    ...
    try:
        resp = requests.get(f"https://otx.alienvault.com/api/v1/indicators/{endpoint}/{indicator}/general", headers=headers, timeout=10)
        if resp.status_code == 200:
...
def fetch_ipinfo_data(ip: str, features: PhishingFeatures):
    ...
    if not IPINFO_TOKEN: return
    headers = {"Authorization": f"Bearer {IPINFO_TOKEN}"}
    try:
        resp = requests.get(f"https://ipinfo.io/{ip}", headers=headers, timeout=10)
        if resp.status_code == 200:
...
def process_domain(features: PhishingFeatures):
    ...
    try:
        cert_resp = requests.get(f"https://crt.sh/?q={features.indicator}&output=json", timeout=10)
        if cert_resp.status_code == 200:
            features.certificate_transparency_velocity = len(cert_resp.json())
            
        fav_resp = requests.get(f"http://{features.indicator}/favicon.ico", timeout=5, verify=False)
...
def process_ip(features: PhishingFeatures):
    ...
    if features.is_bogon_space: return

    # 1. Free AbuseIPDB
    if ABUSEIPDB_KEY:
        try:
            ab_resp = requests.get("https://api.abuseipdb.com/api/v2/check", 
                                   headers={'Key': ABUSEIPDB_KEY, 'Accept': 'application/json'}, 
                                   params={'ipAddress': features.indicator, 'maxAgeInDays': 90}, timeout=10)
...
def analyze_indicator(indicator: str) -> dict:
    ...
    # Respecting free tier rate limits between processing targets
    time.sleep(2) 
    return features
...
if __name__ == "__main__":
    ...
    if args.file:
        try:
            with open(args.file, 'r') as file:
                targets_to_scan.extend(file.readlines())
    ...
    for target in targets_to_scan:
        ...
        final_results.append(asdict(analyze_indicator(target)))
